...
function crop_eyeball

% based on  function crop_jpg2tif, written by Joshua Logan,
% jlogan@usgs.gov (6/25/08) for the 'cobble cam' toolbox, itself created by
% Jon Warrick (USGS) see- [http://walrus.wr.usgs.gov/staff/jwarrick/]
%
...
[image_name, image_path] = uigetfile({'*.tiff';'*.TIF';'*.TIFF';'*.tif';'*.JPG';'*.jpg';'*.BMP';'*.bmp';'*.PNG';'*.png'},...
    'Load images ...', 'MultiSelect', 'on');

mkdir([image_path(1,:),'cropped'])

uiwait(msgbox('Draw a rectangle around the part of the image which is most suitable for grain size analysis (i.e. mineral grains - crop out organic matter, large shells, blur, distortion, etc). When cropping, you may draw rectangles outside the axes of the figure window.',...
    '''crop_eyeball'' Instructions','help','modal'))

if iscell(image_name)
for i = 1

    img=double(col2gray(imread([image_path(1,:),char(image_name(i))])));
    
    disp('------------------------------------------------------------------');
    disp(['Displaying image: ' [image_path(1,:),char(image_name(i))]]);
    disp([num2str(i) ' of ' num2str(length(image_name))]);
    disp('Drag box to crop image');
    warning off all
    [rgb2,rect]=crop_image(img);
    
    answer = questdlg('Crop all images to the same field-of-view?', 'Question', 'No');
    if numel(answer)==3
        action='crop_all';
    else
        action='crop_interactive';
    end
    
    img=img(round(rect(2)):round(rect(4)),round(rect(1)):round(rect(3)));
    
    disp(['Saving as ' [image_path(1,:),'cropped',filesep,char(image_name(i)),'_crop.tif']]);
    imwrite(uint8(img), [image_path(1,:),'cropped',filesep,char(image_name(i)),'_crop.tif'], 'tif');
    
end
close

if numel(action)==8 % i.e. 'crop_all'
for i = 2:length(image_name) 

img=double(imread([image_path(1,:),char(image_name(i))]));
    
    disp('------------------------------------------------------------------');
    disp(['Automatically cropping image: ' [image_path(1,:),char(image_name(i))]]);
    disp([num2str(i) ' of ' num2str(length(image_name))]);
    
    img=double(img(round(rect(2)):round(rect(4)),round(rect(1)):round(rect(3))));
...
close
    ...
    warning on all
    disp(['Saving as ' [image_path(1,:),'cropped',filesep,char(image_name(i)),'_crop.tif']]);
    imwrite(uint8(rgb2), [image_path(1,:),'cropped',filesep,char(image_name(i)),'_crop.tif'], 'tif');
...
close
    ...
    img=double(col2gray(imread([image_path,image_name])));
    
    disp('------------------------------------------------------------------');
    disp(['Displaying image: ' [image_path,image_name]]);
    disp('Drag box to crop image');
    warning off all
    [rgb2,rect]=crop_image(img);
    ...
    disp(['Saving as ' [image_path,'cropped',filesep,image_name,'_crop.tif']]);
    imwrite(uint8(img), [image_path,'cropped',filesep,image_name,'_crop.tif'], 'tif');  
